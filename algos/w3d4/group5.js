...
function getMaxServings(recipe, ingredients) {
  const neededStuffs = Object.keys(recipe);
  var min=Infinity;
  for (const neededStuff of neededStuffs) {
      if(ingredients.hasOwnProperty(neededStuff)) {
          let tempMin = Math.floor(ingredients[neededStuff]/recipe[neededStuff]);
          if (tempMin < min) {
              min = tempMin;
          }
      }
